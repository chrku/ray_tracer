        ...
        for (int b = -11; b < 11; ++b) {
            ...
            if ((center - Vec3(4, 0.2, 0)).norm() > 0.9f) {
                if (choose_mat < 0.8) // diffuse
                    world.addHitable(MovingSphere(center, center + Vec3(0.f, 0.5f * dist(prng), 0.f), 0.f, 1.f, 0.2,
                                                  Lambertian(Vec3(dist(prng) * dist(prng), dist(prng) * dist(prng),
                                                                  dist(prng) * dist(prng)))));
                else if (choose_mat < 0.95) //metal
                    world.addHitable(Sphere(center, 0.2, Metal(Vec3(dist(prng) * dist(prng), dist(prng) * dist(prng),
                                                                    dist(prng) * dist(prng)), 0.5 * dist(prng))));
                else
